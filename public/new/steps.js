...
const createStep = (btns, textValue, txt) => {
    ...
    const newStep = `<div class="step" id="step${num}Div">
        ...
        <label for="step${num}">Step ${num}: </label>
        <textarea type="text" id="step${num}" name="recipe[steps][${num}][step]" required></textarea>
        <button class="delete-btn" type="button" id="deleteStep${num}" value="${num}">x</button>
        </div>`;
...
const updateHTML = (btns, textValue, txt, stepValue) => {
    for (let i = 0; i < btns.length; i++) {
        ...
        if (parsedValue > stepValue) {
            ...
            <label for="step${j}">Step ${j}: </label>
            <textarea type="text" id="step${j}" name="recipe[steps][${j}][step]" required></textarea>
            <button class="delete-btn" type="button" id="deleteStep${j}" value="${j}">x</button>
            </div>`;
        }
    } for (let i = 0; i < btns.length; i++) {
        txt[i].value = textValue[i]
